...
class Clocked(hds.HDS):
  def __init__(self, config):
    """
    Clocked clocked piecewise-defined model
    """
    self.p = config
    self.name = 'Clocked'
    ...
    self.x0 = self.p['x0']
    self.q0 = self.p['q0']
    self.accel = lambda time,state,pars : np.zeros((state.shape[0],3))
    self.period = self.p['period']
    self.potential = self.p['potential']
    self.dpotential = self.p['dpotential']
    self.mass = self.p['mass']


  def dyn(self, time, state, pars):
    """
    .dyn  evaluates system dynamics from (*)

      (*) M(q) ddq = dV(q)
    """
    state = np.asarray(state).flatten()
    # perturbation
    acc = self.accel(time,np.asarray([state]),pars).flatten()
    # potential field
    pot = self.dpotential(time,np.asarray([state]),pars).flatten()
    # inertia tensor
    mass = self.mass(time,np.asarray([state]),pars).flatten()
    # unpack state into positions and velocities
    d = state.size/2
    q,dq = state[:d],state[d:]
    # return time derivative of state computed from (*)
    dstate = np.r_[dq,np.dot(acc + pot,np.linalg.inv(mass))]
    return dstate
  ...
  def obs(self):
    ...
    """
    o = hds.Obs(t=self.time,q=[],dq=[],acc=[],pot=[],dyn=[])

    for time,state,pars in zip(self.time,self.state,self.pars):
      # perturbation
      acc = self.accel(time,state,pars)
      # potential field
      pot = self.dpotential(time,state,pars)
      # potential field
      dyn = self.dyn(time,state,pars)
      # pre-allocate
      ones = np.ones((state.shape[0],1))
      # unpack state into positions and velocities
      d = state.shape[1]/2
      q,dq = state[:,:d],state[:,d:]
  ...
  def evts(self, pars):
    ...
    """
    return [lambda t,x : pars[2]*pars[1] - t]
